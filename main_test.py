...
def main():
    ...
    parser.add_argument('--task', type=str, default='001_DAAVSR_videosr_bi_REDS_15frames', help='tasks: 001 to 003')
    parser.add_argument('--sigma', type=int, default=0, help='noise level for denoising: 10, 20, 30, 40, 50')
    parser.add_argument('--folder_lq', type=str, default='testsets/REDS4/val_sharp_bicubic',
...
def prepare_model_dataset(args):
    ...
    # define model
    if args.task == '001_DAAVSR_videosr_bi_REDS_30frames':
    ...
    elif args.task in ['002_DAAVSR_videosr_bi_Vimeo_14frames', '003_RVRT_videosr_bd_Vimeo_14frames']:
        model = net(upscale=4, clip_size=2, img_size=[2, 64, 64], window_size=[2, 8, 8], num_blocks=[1, 2, 1],
                    depths=[2, 2, 2], embed_dims=[144, 144, 144], num_heads=[6, 6, 6],
                    inputconv_groups=[1, 1, 1, 1, 1, 1], deformable_groups=12, attention_heads=12,
                    attention_window=[3, 3], cpu_cache_length=100)
    ...
    # load model
    model_path = f'model_zoo/DAAVSR/{args.task}.pth'
    os.path.exists(model_path):
    print(f'loading model from ./model_zoo/DAAVSR/{model_path}')
    ...
    # load datasets
    os.path.exists(f'{args.folder_lq}'):
